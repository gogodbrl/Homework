...
public class MyFrequency extends Configured implements Tool {
	private static final String OUTPUT_PATH = "1-word-freq";
	private static final String INPUT_PATH = "shakespeare";
	...
	public int run(String[] args) throws Exception {
		...
		FileInputFormat.addInputPath(job, new Path(INPUT_PATH)); //경로
	...
	public static class WordFrequenceInDocMapper extends
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			FileSplit fileSplit = (FileSplit) context.getInputSplit(); //term split
			String docid = fileSplit.getPath().getName(); //why number.

			Matcher m = PATTERN.matcher(value.toString()); //text set
			while (m.find()) {
	...
	public static class WordFrequenceInDocReducer extends
		...
		private IntWritable tf = new IntWritable(); //int write value
		...
		protected void reduce(Text key, Iterable<IntWritable> values, Context context)
			...
			for (IntWritable val : values)
				sum += val.get(); //how know same

			tf.set(sum);
			context.write(key, tf); //context write something.
		}
